...
described in: https://arxiv.org/abs/xxxx.xxxxx.
"""
...
def init(args):
    ...
    assert hasattr(models, args.model), "Model {} is not available in torchvision.models.".format(args.model)
...
class FCN(nn.Module):
    ...
    def __init__(
        self, input_channels=3, output_channels=3, kernel_size=3, channels=32, activation=True, dropout=True, bias=True
    ):
...
class RIDE:
    ...
    def defense(self, image, cln_image=None, classifier=None, label=None):
        ...
        if self.args.bs > 1:
            batch_input_size = (self.args.bs, self.input_size[1], self.input_size[2], self.input_size[3])
        ...
        optimizer = torch.optim.Adam(
            model.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-5, amsgrad=True
        )
